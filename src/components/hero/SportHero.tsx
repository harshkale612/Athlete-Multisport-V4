...
const SportHero = ({ sportKey }: SportHeroProps) => {
  ...
  return (
    <section
      className="relative min-h-[90vh] flex flex-col justify-end pt-32 pb-16 px-6 lg:px-24 overflow-hidden"
      aria-label={`${sport.name} hero`}
    >
      <div className="absolute inset-0 z-0">
        <Image
          src={sport.image || 'https://images.unsplash.com/photo-1541534741688-6078c6bfb5c5?w=1000'}
          alt={sport.imageAlt || `${sport.name} – Harsh Kale`}
          fill
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto flex flex-col md:flex-row items-end justify-between gap-12">
        ...
        <motion.div
          ...
          aria-label={`${sport.name} key statistics`}
        >
          {sportStats.slice(0, 2).map((stat, idx) => (
            <div
              key={idx}
              className="flex-1 border-r last:border-0 border-contrast-alpha/20 px-4 first:pl-0 last:pr-0"
            >
              <span className="text-4xl font-black text-text block mb-1 font-mono">
                {stat.value}
              </span>
