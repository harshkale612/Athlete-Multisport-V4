...
const About = () => {
  const highlights = athleteProfile.keyHighlights || [];

  return (
    <section
      id="about"
      className="py-24 px-6 md:px-12 lg:px-24 bg-background border-t border-contrast-alpha/5 relative overflow-hidden"
    ...
    >
      <div className="max-w-5xl mx-auto flex flex-col md:flex-row gap-12 lg:gap-24 items-center">
        ...
        <div className="flex-1 space-y-8">
          ...
          >
            <p>{athleteProfile.bio || athleteProfile.shortBio}</p>
          </motion.div>
        </div>
        ...
        <div className="flex-1 w-full grid grid-cols-2 gap-4">
          {highlights.map((highlight, idx) => (
            <motion.div
              ...
              className="bg-surface/50 backdrop-blur-md border border-contrast-alpha/10 p-6 rounded-2xl flex flex-col items-center justify-center text-center hover:bg-surface/70 transition-colors"
            >
              <span className="text-3xl font-black text-text font-mono mb-2">
                {highlight.value}
              </span>
