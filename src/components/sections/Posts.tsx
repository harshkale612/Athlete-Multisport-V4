...
const Posts = ({ sportKey }: PostsProps) => {
  const filteredPosts = sportKey
    ? postsData.filter(
        (p: Post) => p.sport === sportKey || p.sport === 'multi' || !p.sport
      )
    : postsData;
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-16">
          ...
          >
            ...
            >
              Latest from{' '}
              <span className="text-transparent text-edge-outline">YouInSports</span>
            </h2>
          </motion.div>
          ...
          <motion.a
            ...
            aria-label="Follow Harsh Kale on YouInSports"
          >
            Follow Athlete
            <ExternalLink
              size={18}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {publicPosts.map((post: Post, index: number) => (
            <motion.article
              key={post.id}
            ...
            >
              <div className="aspect-square w-full relative overflow-hidden border-b border-contrast-alpha/10">
                ...
                <div className="absolute inset-x-0 bottom-0 top-1/2 bg-gradient-to-t from-black/80 via-black/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end p-6">
                  <p className="text-text text-sm line-clamp-3 leading-relaxed drop-shadow-lg">
                    {post.caption}
                  </p>
                </div>
              </div>

              <footer className="p-4 flex items-center justify-between text-text-muted">
                <div className="flex gap-4">
                  <span className="flex items-center gap-1.5 text-xs font-bold hover:text-primary transition-colors">
                    <Heart size={16} aria-hidden="true" />
                    <span aria-label={`${post.likesCount} likes`}>{post.likesCount}</span>
                  </span>
