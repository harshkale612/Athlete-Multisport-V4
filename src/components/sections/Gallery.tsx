...
const Gallery = ({ sportKey }: GalleryProps) => {
  const [selectedImage, setSelectedImage] = useState<GalleryItem | null>(null);

  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const images: GalleryItem[] = sportKey
    ? (galleryData as any[]).filter(
        (item) => item.type === 'image' && (item.sportKey === sportKey || item.sportKey === 'multi')
      )
    : (galleryData as any[]).filter((item) => item.type === 'image');
      ...
      <AnimatePresence>
        {selectedImage && (
          <motion.div
            ...
            role="dialog"
            aria-modal="true"
            aria-label={`Photo: ${selectedImage.title}`}
          >
            <button
              onClick={(e) => {
                e.stopPropagation();
