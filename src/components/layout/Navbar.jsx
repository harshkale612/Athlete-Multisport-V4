    ...
    <>
      ...
      >
        <div className="max-w-7xl mx-auto px-6 lg:px-12 flex justify-between items-center">
          ...
          {/* Logo */}
          <Link to="/" className="text-2xl font-display font-black tracking-tighter text-text z-50">
            {athleteProfile.name.split(' ')[0]}<span className="text-primary">.</span>
          </Link>

          {/* Desktop Navigation */}
          <nav className="hidden md:flex items-center gap-8">
            <Link to="/" className="text-sm font-bold uppercase tracking-widest text-text hover:text-primary transition-colors">
              Home
            </Link>
            
            {/* Sports Dropdown Alternative (Just links for now) */}
            ...
            <div className="flex bg-surface/50 border border-contrast-alpha/10 rounded-full p-1 backdrop-blur-sm">
              ...
              <button 
                ...
                className={`p-2 rounded-full transition-colors ${theme === 'light-editorial' ? 'bg-primary text-white' : 'text-text-muted hover:text-text'}`}
                aria-label="Light Theme"
              >
                <Sun size={16} />
              </button>
               {/* Optional adapt to sport theme */}
            </div>
          </nav>
        ...
        {isMobileMenuOpen && (
          ...
          >
            <nav className="flex flex-col items-center gap-8 text-center mt-12 w-full">
              <Link to="/" className="text-3xl font-display font-black uppercase text-text">
                Home
              </Link>
              ...
              {sportsData.map(sport => (
               ...
               <div className="mt-8 flex gap-4 bg-surface rounded-full p-2 border border-contrast-alpha/10">
                 ...
                 <button 
                  onClick={() => toggleTheme('light-editorial')}
                  className={`p-3 rounded-full ${theme === 'light-editorial' ? 'bg-primary text-white' : 'text-text-muted'}`}
                 >
                   <Sun size={20} />
                 </button>
               </div>
